...
export default function JarList() {
  ...
  return (
    <div className="space-y-6">
      {/* Create Jar */}
      <form onSubmit={createJar} className="rounded-2xl border p-4 bg-white/90 backdrop-blur shadow-sm">
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex-1 min-w-[240px]">
            <label className="text-xs font-semibold text-gray-600">New Jar Name</label>
            <input
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <div className="p-2 rounded-xl bg-violet-50 text-violet-700"><LuWallet /></div>
                  <div>
                    <div className="font-semibold">{j.name}</div>
                    <div className="text-xs text-gray-500">Jar</div>
                  </div>
                </div>
                <button
                  onClick={() => removeJar(j._id, balance)}
                  className={`p-2 rounded-lg ${balance > 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-red-50 text-red-600 hover:bg-red-100'}`}
              ...
              <div className="mt-3 text-sm text-gray-600">Balance</div>
              <div className="text-xl font-semibold">{format(balance)}</div>

              <div className="mt-4 flex gap-2 items-end">
                <div className="flex-1">
                  <label className="text-xs font-semibold text-gray-600">Amount (THB)</label>
                  <input
                    className="mt-1 w-full border rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-violet-500"
                    value={amt}
                ...
                <button onClick={() => fund(j._id)} className="px-3 py-2 rounded-xl bg-green-600 text-white hover:bg-green-700 flex items-center gap-1" title="Move from free cash → jar">
                  <LuArrowUp /> Fund
                </button>
                <button onClick={() => withdraw(j._id)} className="px-3 py-2 rounded-xl bg-amber-600 text-white hover:bg-amber-700 flex items-center gap-1" title="Move from jar → free cash">
        ...
        {jars.length === 0 && <div className="col-span-full text-sm text-gray-500">No jars yet. Create one above.</div>}
      </div>
    </div>
  );
}
