...
const BulkDeleteIncome = ({ isOpen, onClose, onConfirm, isDarkTheme }) => {
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-r from-red-500 to-pink-400 rounded-xl p-4 shadow-lg">
        <h3 className="text-sm font-bold text-white">{tt('income.warning','Warning')}</h3>
      ...
      <p className="font-bold text-lg">{tt('income.selectDeletionPeriod',"Select deletion period:")}</p>
      <div className="space-y-3">
        {[
          { label: tt('income.lastMonth','Last Month'), value: 'last-month', color: 'blue' },
          { label: tt('income.last6Month','Last 6 Months'), value: 'last-6-months', color: 'indigo' },
          { label: tt('income.lastYear','Last Year'), value: 'last-year', color: 'purple' },
          { label:  tt('income.allIncome','All Income'), value: 'all', color: 'red' },
      ...
      <div className="flex justify-end gap-3 pt-4 border-gray-300">
        <button
          onClick={onClose}
