...
const ProfilePhotoSelector = ({ photo, onUpload, onRemove }) => {
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button type="button" onClick={triggerFileSelect} disabled={isUploading} className="inline-flex items-center justify-center px-4 py-2.5 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-400 text-white font-medium rounded-lg transition-colors duration-200">
          {isUploading ? "Uploading..." : preview ? "Change Photo" : "Upload Photo"}
        </button>

        {preview && !isUploading && (
          <button type="button" onClick={handleRemove} className="inline-flex items-center justify-center px-4 py-2.5 bg-white hover:bg-red-50 border border-red-300 text-red-700 hover:text-red-800 font-medium rounded-lg transition-colors duration-200">
            Remove Photo
          </button>
        )}
      </div>
