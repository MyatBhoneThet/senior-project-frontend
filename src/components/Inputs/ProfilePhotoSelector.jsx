  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <div className="relative group">
        <div className={`w-32 h-32 rounded-full border-4 overflow-hidden flex items-center justify-center shadow-lg ${
          isDarkTheme ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-gray-50"
        }`}>
          {preview ? (
            <img src={preview} alt={tt("profilePhoto.alt", "Profile")} className="w-full h-full object-cover" />
          ) : (
      ...
      <p className={`text-sm text-center max-w-xs ${isDarkTheme ? "text-gray-400" : "text-gray-500"}`}>
        {tt("profile.helperText", "Choose a clear photo that represents you well. JPG, PNG, or GIF format.")}
      </p>
    </div>
  );
};
