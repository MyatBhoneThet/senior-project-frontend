...
const serverWeekToUI = (v) => {
  if (!v) return 'Sun';
  const s = String(v).trim().toLowerCase();
  return s.startsWith('mon') ? 'Mon' : 'Sun';
};
      ...
      <div className={`max-w-4xl mx-auto p-6 space-y-8 overflow-y-auto ${containerClass}`}>
        ...
        <div className={cardClass}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={resetPreferences} className={buttonSecondaryClass}>
              🔄 {tt('settings.reset', 'Reset to Defaults')}
            </button>
          </div>
        </div>
        ...
        <div className={cardClass}>
          ...
          <div className="space-y-4">
            <div>
              <label className={`block text-sm font-medium mb-1 ${labelBaseClass}`}>{tt('settings.currentPassword', 'Current Password')}</label>
              <input type="password" value={currPwd} onChange={(e) => setCurrPwd(e.target.value)} className={inputBaseClass} />
            </div>

            <div>
              <label className={`block text-sm font-medium mb-1 ${labelBaseClass}`}>{tt('settings.newPassword', 'New Password')}</label>
              <input type="password" value={newPwd} onChange={(e) => setNewPwd(e.target.value)} className={inputBaseClass} />
            </div>

            <div>
              <label className={`block text-sm font-medium mb-1 ${labelBaseClass}`}>{tt('settings.confirmNewPassword', 'Confirm New Password')}</label>
              <input type="password" value={confirmPwd} onChange={(e) => setConfirmPwd(e.target.value)} className={inputBaseClass} />
            </div>

            <button onClick={handleChangePassword} disabled={changingPwd} className={`${buttonPrimaryClass} ${changingPwd ? 'opacity-50 cursor-not-allowed' : ''}`}>
              🔑 {changingPwd ? tt('settings.changingPassword', 'Updating...') : tt('settings.changePassword', 'Change Password')}
            </button>
          </div>
        </div>
        ...
        <div className={cardClass}>
          ...
          <button
            ...
            disabled={deleting}
            className={`${buttonPrimaryClass} mt-3 bg-red-600 hover:bg-red-700 focus:ring-red-500 ${deleting ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            🗑️ {deleting ? tt('settings.deleting', 'Deleting...') : tt('settings.deleteBtn', 'Delete Account')}
          </button>
        </div>
      </div>
    </DashboardLayout>
