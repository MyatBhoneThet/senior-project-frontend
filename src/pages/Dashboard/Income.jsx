...
const Income = () => {
  ...
  const handleAddIncome = async (income) => {
    ...
    if (!amount || isNaN(amount) || Number(amount) <= 0)
      return toast.error(
        tt('income.text2', 'Amount must be greater than 0.')
      );
    if (!date)
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          tt('income.text5', 'Something went wrong.')
      );
    }
  };
  ...
  const handleUpdateIncome = async (payload) => {
    ...
    try {
      await axiosInstance.put(
        API_PATHS.INCOME.UPDATE_INCOME(selectedIncome._id),
        {
  ...
  const handleBulkDelete = async (period) => {
    try {
      const data = await BulkDeleteIncome(period); // must return Promise
      setOpenBulkDeleteModal(false);
      toast.success(
        data?.message || tt('income.text8', 'Income deleted successfully.')
      );
      await fetchIncomeDetails();
    } catch (error) {
      toast.error(
        error?.message || tt('income.text5', 'Something went wrong.')
      );
    }
  };
