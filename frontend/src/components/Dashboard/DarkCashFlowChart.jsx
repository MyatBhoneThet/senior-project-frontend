  ...
  if (active && payload && payload.length) {
    return (
      <div className={`rounded-lg border p-3 shadow-xl backdrop-blur-xl ${isDark ? 'border-white/10 bg-white/[0.08]' : 'border-white/35 bg-white/45'}`}>
        <p className={`mb-2 text-xs font-medium tracking-widest ${isDark ? 'text-gray-400' : 'text-[#6b7080]'}`}>{label}</p>
  ...
  return (
    <div className={`h-[340px] w-full overflow-hidden rounded-[22px] border p-5 flex flex-col relative ${cardClass}`}>
      <div className="flex justify-between items-center mb-4 z-10">
        <h3 className={`text-[13px] font-medium tracking-[0.1em] ${isDark ? 'text-gray-100' : 'text-[#11131b]'}`}>Cash Flow</h3>
        <div className="flex gap-3 text-[11px]">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(132,204,22,0.4)] ${isDark ? 'bg-[#a3e635]' : 'bg-[#84cc16]'}`} />
            <span className={`tracking-wider ${isDark ? 'text-gray-400' : 'text-[#6b7080]'}`}>Income</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-[#fb7185] shadow-[0_0_8px_rgba(251,113,133,0.8)]" />
            <span className={`tracking-wider ${isDark ? 'text-gray-400' : 'text-[#6b7080]'}`}>Expense</span>
          </div>
        </div>
      </div>
