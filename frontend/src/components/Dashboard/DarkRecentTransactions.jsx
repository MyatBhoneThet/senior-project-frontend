  ...
  return (
    <div className={`h-[340px] w-full overflow-hidden rounded-[22px] border p-5 flex flex-col relative ${cardClass}`}>
      <div className="flex justify-between items-center mb-4">
        <h3 className={`text-[13px] font-medium tracking-[0.1em] ${isDark ? 'text-gray-100' : 'text-[#11131b]'}`}>{tt('dashboard.recent', 'Recent')}</h3>
        <button onClick={onSeeAll} className={`rounded px-3 py-1 text-[10px] tracking-widest transition-colors ${isDark ? 'bg-[#9b51e0]/18 text-[#cbb2ff] hover:bg-[#9b51e0]/26' : 'bg-[#f2e9ff]/70 text-[#7e3af2] hover:bg-[#f2e9ff]/90'}`}>
      ...
      <div className="flex flex-col gap-3 overflow-y-auto scrollbar-none pb-2 h-full">
        {transactions?.length > 0 ? (
          transactions.map((t, idx) => (
            <div key={idx} className={`-mx-2 flex items-center justify-between rounded-xl p-1.5 transition-colors group cursor-pointer ${isDark ? 'hover:bg-white/[0.02]' : 'hover:bg-black/[0.03]'}`}>
              <div className="flex items-center gap-3">
                <div className={`flex h-9 w-9 items-center justify-center rounded-xl border transition-colors ${isDark ? 'border-white/10 bg-white/[0.05] group-hover:bg-white/[0.08]' : 'border-black/6 bg-[rgba(17,19,27,0.04)] group-hover:bg-[rgba(17,19,27,0.08)]'}`}>
                  {getIconForCategory(t.category, t.type)}
                </div>
                <div className="flex flex-col">
                  <span className={`text-xs font-medium ${isDark ? 'text-gray-200' : 'text-[#11131b]'}`}>{t.source || t.category || tt('dashboard.transactionFallback', 'Transaction')}</span>
                  <span className={`mt-0.5 text-[9px] font-medium tracking-widest ${isDark ? 'text-gray-500' : 'text-[#6b7080]'}`}>{t.category || tt('dashboard.generalCategory', 'GENERAL')}</span>
                </div>
              </div>
          ...
          <div className={`flex h-full flex-col items-center justify-center ${isDark ? 'text-gray-500' : 'text-[#6b7080]'}`}>
            ...
            <p className="text-[11px] font-medium tracking-widest">{tt('dashboard.noTransactions', 'No recent transactions')}</p>
          </div>
        )}
      </div>
    </div>
