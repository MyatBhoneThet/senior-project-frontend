  ...
  return (
    <div className={`relative overflow-hidden rounded-[24px] border p-6 ${cardClass}`}>
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        <div>
          <h3 className={`mb-3 text-[11px] font-medium tracking-[0.15em] ${isDark ? 'text-gray-500' : 'text-[#6b7080]'}`}>
            {title}
          </h3>
          <div className={`text-4xl font-medium tracking-tighter ${theme.text} mb-1 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]`}>
            {amount}
          </div>
        ...
        {badgeText && (
          <div className="mt-8 flex items-center">
            <span className={`inline-flex items-center px-2.5 py-1 ${theme.badgeBg} ${theme.badgeTextFill} text-[10px] font-medium tracking-wider rounded-md backdrop-blur-sm border border-current/10`}>
              {badgeText}
            </span>
          </div>
        )}
