...
export default function GoalList() {
  ...
  const fund = async (id, amt) => {
    ...
    try {
      ...
      toast.success(data?.message || tt('savings.goalFundedAmount', `Funded THB ${n.toLocaleString()}`));
      await load();
    } catch (err) {
  ...
  const autoAlloc = async (amt) => {
    ...
    try {
      ...
      toast.success(tt('savings.autoAllocated', 'Auto-allocated'));
      await load();
    } catch (err) {
      toast.error(err?.response?.data?.message || err.message || tt('savings.autoAllocateFailed', 'Auto-allocate failed'));
    }
  };

  const removeGoal = async (id) => {
    if (!confirm(tt('savings.confirmDeleteGoal', 'Delete this goal? This cannot be undone.'))) return;
    try {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={create} className={`rounded-2xl border p-4 shadow-sm ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.goalTitle', 'Goal title')}</label>
            <input className={`${inputClass} placeholder-gray-400`} value={title} onChange={e=>setTitle(e.target.value)} placeholder={tt('savings.goalTitlePlaceholder', 'e.g., Buy MacBook')} />
          </div>
          <div>
            <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.targetAmount', 'Target amount (THB)')}</label>
            <input className={`${inputClass} placeholder-gray-400`} value={targetAmount} onChange={e=>setTargetAmount(e.target.value)} placeholder={tt('savings.targetAmountPlaceholder', '35000')} />
          </div>
          <div>
            <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.targetDate', 'Target date')}</label>
            <input className={inputClass} type="date" value={targetDate} onChange={e=>setTargetDate(e.target.value)} />
          </div>
          <div>
            <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.jar', 'Jar')}</label>
          ...
          <div className={`md:col-span-2 p-3 flex flex-wrap items-center gap-3 rounded-xl ${isDark ? 'bg-gray-800' : 'bg-gray-50'}`}>
            <label className="flex items-center gap-2">
              ...
              <span className={`text-sm ${isDark ? 'text-gray-300' : ''}`}>{tt('savings.autoAllocate', 'Enable auto-allocate on income')}</span>
            </label>
            ...
            <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              {tt('savings.goalText2', 'Auto-allocation moves a portion of each income into this goal’s jar.')}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4">
          <button className="px-4 py-2 rounded-xl bg-violet-600 text-white shadow hover:bg-violet-700 flex items-center gap-2">
            <LuPlus /> {tt('savings.createGoal', 'Create goal')}
          </button>
        </div>
      </form>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {goals.map(g => {
          ...
          return (
            <div key={g._id} className={cardClass}>
              ...
              <div className={`mt-3 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{tt('savings.progress', 'Progress')}</div>
              <div className={`text-sm font-medium ${isDark ? 'text-gray-200' : ''}`}>
                {format(saved)} <span className={`text-gray-500 ${isDark ? 'text-gray-400' : ''}`}>{tt('savings.of', 'of')}</span> {format(target)} • {pct}%
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={() => fund(g._id, 500)} className={btnClass('bg-green-600','bg-green-700')}>{tt('savings.fund500', 'Fund ฿500')}</button>
                <button onClick={() => autoAlloc(2000)} className={btnClass('bg-indigo-600','bg-indigo-700')} title={tt('savings.testAutoAllocateTitle', 'Simulate income allocation (e.g., 10% of ฿2000 → jar)')}>
                  {tt('savings.testAutoAllocate', 'Test Auto-Allocate (฿2000)')}
                </button>
              </div>
            </div>
          );
        ...
        {goals.length === 0 && (
          <div className={`col-span-full text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
            {tt('savings.noGoal', 'No goals yet. Create one above.')}
          </div>
        )}
      </div>
    </div>
