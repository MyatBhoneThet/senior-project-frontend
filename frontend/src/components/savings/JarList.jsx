...
export default function JarList() {
  ...
  const createJar = async (e) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.message || e.message || tt('savings.createJarFailed', 'Failed to create jar'));
    } finally {
      setCreating(false);
    }
  };
  ...
  const fund = async (id) => {
    ...
    try {
      ...
      onChangeAmt(id, '');
      toast.success(tt('savings.fundedAmount', `Funded THB ${amt.toLocaleString()}`));
      await load();
    } catch (e) {
      toast.error(e?.response?.data?.message || e.message || tt('savings.fundFailed', 'Fund failed'));
    }
  };
  ...
  const withdraw = async (id) => {
    ...
    try {
      ...
      onChangeAmt(id, '');
      toast.success(tt('savings.withdrewAmount', `Withdrew THB ${amt.toLocaleString()}`));
      await load();
    } catch (e) {
      toast.error(e?.response?.data?.message || e.message || tt('savings.withdrawFailed', 'Withdraw failed'));
    }
  };
  ...
  const removeJar = async (id, balance) => {
    ...
    if (!confirm(tt('savings.confirmDeleteJar', 'Delete this jar? This cannot be undone.'))) return;
    try {
      await axiosInstance.delete(`${API_PATHS.JARS.BASE}/${id}`);
      toast.success(tt('savings.jarDeleted', 'Jar deleted'));
      await load();
    } catch (e) {
      toast.error(e?.response?.data?.message || e.message || tt('savings.deleteFailed', 'Delete failed'));
    }
  };
      ...
      <form onSubmit={createJar} className={`rounded-2xl border p-4 shadow-sm backdrop-blur ${isDark ? 'bg-gray-900/90 border-gray-700' : 'bg-white/90 border-gray-300'}`}>
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex-1 min-w-[240px]">
            <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.newJarName', 'New Jar Name')}</label>
            <input
          ...
          >
            {creating ? tt('savings.addingJar', 'Adding…') : tt('savings.addJar', 'Add Jar')}
          </button>
        </div>
      </form>
        ...
        {jars.map(j => {
          ...
          return (
            <div key={j._id} className={cardClass}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{tt('savings.jar', 'Jar')}</div>
                  </div>
                </div>
                <button
              ...
              <div className={`mt-3 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{tt('savings.balance', 'Balance')}</div>
              <div className={`text-xl font-semibold ${isDark ? 'text-white' : ''}`}>{format(balance)}</div>

              <div className="mt-4 flex gap-2 items-end">
                <div className="flex-1">
                  <label className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{tt('savings.amount', 'Amount (THB)')}</label>
                  <input
                ...
                <button onClick={() => fund(j._id)} className={btnClass('bg-green-600','bg-green-700') } title="Move from free cash → jar">
                  <LuArrowUp /> {tt('savings.fund', 'Fund')}
                </button>
                <button onClick={() => withdraw(j._id)} className={btnClass('bg-amber-600','bg-amber-700')} title="Move from jar → free cash">
                  <LuArrowDown /> {tt('savings.withdraw', 'Withdraw')}
                </button>
              </div>
            </div>
          );
        })}
        {jars.length === 0 && <div className={`col-span-full text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{tt('savings.noJars', 'No jars yet. Create one above.')}</div>}
      </div>
    </div>
  );
}
