...
const ExpenseStatCards = ({ statCards, isDark, cardClass, labelText, mutedText, isLoading = false }) => (
    ...
    {statCards.map((card) => (
      <div key={card.title} className={`relative overflow-hidden rounded-[22px] border p-6 ${cardClass}`}>
        ...
        <div className="relative">
          <div className={`mb-4 text-[13px] font-semibold ${labelText}`}>{card.title}</div>
          <div className={`text-4xl font-bold tracking-tight ${card.accent}`}>{card.value}</div>
